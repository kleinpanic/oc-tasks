...
import os from 'os';
import { evaluateBackburner, recommendModel } from './models/detail-score.js';
...
export function seedDefaultSLAPolicies(db) {
  ...
  `);
    for (const p of policies) {
        insert.run(p.id, p.name, p.description, JSON.stringify(p.condition), JSON.stringify(p.action), p.enabled ? 1 : 0, now);
    }
}
function logActivity(db, taskId, action, actor, oldValue, newValue) {
    db.prepare(`
...
export function createTask(db, input, actor = 'cli') {
    const now = new Date().toISOString();
    const id = uuidv4();
  ...
  `).run(id, input.title, input.description ?? '', status, priority, complexity, danger, input.type ?? 'manual', input.assignedTo ?? null, input.list ?? 'personal', JSON.stringify(input.tags ?? []), backburner.detailScore, backburner.minDetailRequired, backburner.shouldBackburner ? 1 : 0, JSON.stringify([]), backburner.shouldBackburner ? backburner.message : '', input.dueDate ?? null, 0, input.estimatedMinutes ?? null, 0, null, null, null, input.parentId ?? null, input.projectId ?? null, now, now, null, now, input.source ?? 'cli', JSON.stringify(input.metadata ?? {}), recModel);
...
export function findTaskBySubstring(db, substr) {
    // Try exact UUID match first
    const exact = getTask(db, substr);
    if (exact)
        return exact;
    // Try UUID prefix match
    const prefix = db.prepare('SELECT * FROM tasks WHERE id LIKE ?').get(`${substr}%`);
    if (prefix)
        return rowToTask(prefix);
    // Try title substring match (case insensitive)
    const titleMatch = db.prepare(`SELECT * FROM tasks WHERE title LIKE ? AND status != 'archived' ORDER BY updatedAt DESC LIMIT 1`).get(`%${substr}%`);
    if (titleMatch)
        return rowToTask(titleMatch);
    return null;
}
...
export function listTasks(db, opts = {}) {
    ...
    const rows = db.prepare(`SELECT * FROM tasks ${where} ${orderBy} ${limitClause}`).all(...params);
    return rows.map(rowToTask);
}
export function updateTask(db, id, updates, actor = 'cli') {
...
export function moveTask(db, id, status, actor = 'cli') {
    return updateTask(db, id, { status }, actor);
}
export function completeTask(db, id, actor = 'cli') {
    return moveTask(db, id, 'completed', actor);
}
export function blockTask(db, id, reason, actor = 'cli') {
    return updateTask(db, id, { status: 'blocked', blockerDescription: reason }, actor);
}
export function unblockTask(db, id, actor = 'cli') {
    const now = new Date().toISOString();
    db.prepare(`
...
export function searchTasks(db, pattern) {
    const rows = db.prepare(`SELECT * FROM tasks WHERE (title LIKE ? OR description LIKE ?) AND status != 'archived' ORDER BY updatedAt DESC`).all(`%${pattern}%`, `%${pattern}%`);
    return rows.map(rowToTask);
}
export function getOverdueTasks(db) {
    const today = new Date().toISOString().slice(0, 10);
    const rows = db.prepare(`SELECT * FROM tasks WHERE dueDate < ? AND status NOT IN ('completed', 'archived')
     ORDER BY dueDate ASC`).all(today);
    return rows.map(rowToTask);
}
export function getNextTask(db, agent) {
    const conditions = [
        "status = 'ready'",
        "autoBackburnered = 0",
    ];
...
export function getTaskActivity(db, taskId) {
    return db.prepare('SELECT * FROM task_activity WHERE taskId = ? ORDER BY timestamp DESC').all(taskId);
}
// Timer operations
export function startTimer(db, taskId, agent) {
    const id = uuidv4();
    const now = new Date().toISOString();
    db.prepare('INSERT INTO timer_sessions (id, taskId, agent, startedAt) VALUES (?, ?, ?, ?)').run(id, taskId, agent ?? null, now);
    return id;
}
export function stopTimer(db, timerId, taskId) {
    let session;
    if (timerId) {
        session = db.prepare('SELECT * FROM timer_sessions WHERE id = ? AND stoppedAt IS NULL').get(timerId);
    }
    else if (taskId) {
        session = db.prepare('SELECT * FROM timer_sessions WHERE taskId = ? AND stoppedAt IS NULL ORDER BY startedAt DESC LIMIT 1').get(taskId);
    }
    else {
        session = db.prepare('SELECT * FROM timer_sessions WHERE stoppedAt IS NULL ORDER BY startedAt DESC LIMIT 1').get();
    }
    if (!session)
        return null;
    const now = new Date();
    ...
    db.prepare('UPDATE timer_sessions SET stoppedAt = ?, totalSeconds = ? WHERE id = ?').run(now.toISOString(), totalSeconds, session.id);
    // Update task's actual minutes
    db.prepare('UPDATE tasks SET actualMinutes = actualMinutes + ? WHERE id = ?').run(Math.ceil(totalSeconds / 60), session.taskId);
    return { ...session, stoppedAt: now.toISOString(), totalSeconds };
}
export function getRunningTimers(db) {
    return db.prepare(`
...
export function getTimerReport(db, days = 7) {
...
// Stats
export function getStats(db, days = 30) {
    const cutoff = new Date(Date.now() - days * 86400_000).toISOString();
    const total = db.prepare('SELECT COUNT(*) as cnt FROM tasks').get().cnt;
    const byStatus = db.prepare('SELECT status, COUNT(*) as cnt FROM tasks GROUP BY status').all();
    const completed = db.prepare('SELECT COUNT(*) as cnt FROM tasks WHERE completedAt >= ?').get(cutoff).cnt;
    const created = db.prepare('SELECT COUNT(*) as cnt FROM tasks WHERE createdAt >= ?').get(cutoff).cnt;
    const backburnered = db.prepare('SELECT COUNT(*) as cnt FROM tasks WHERE autoBackburnered = 1').get().cnt;
    const overdue = getOverdueTasks(db).length;
    const byAgent = db.prepare(`SELECT assignedTo, COUNT(*) as cnt FROM tasks
     WHERE assignedTo IS NOT NULL AND status NOT IN ('completed', 'archived')
     GROUP BY assignedTo`).all();
    const byList = db.prepare('SELECT list, COUNT(*) as cnt FROM tasks GROUP BY list').all();
