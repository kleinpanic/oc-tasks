...
export function evaluatePolicies(db) {
    const breaches = [];
    const now = new Date().toISOString();
    const today = now.slice(0, 10);
    const policiesRaw = db.prepare('SELECT * FROM sla_policies WHERE enabled = 1').all();
    ...
    for (const policy of policies) {
        ...
        let tasks = [];
        if (cond.overdue) {
            tasks = db.prepare(`SELECT id, title FROM tasks WHERE dueDate < ? AND status NOT IN ('completed', 'archived')`).all(today);
        }
        else if (cond.countGreaterThan != null && cond.status) {
            const count = db.prepare(`SELECT COUNT(*) as cnt FROM tasks WHERE status = ?`).get(cond.status);
            if (count.cnt > cond.countGreaterThan) {
                tasks = db.prepare(`SELECT id, title FROM tasks WHERE status = ?`).all(cond.status);
            }
        }
        else if (cond.status && cond.olderThanHours) {
            const cutoff = new Date(Date.now() - cond.olderThanHours * 3600_000).toISOString();
            tasks = db.prepare(`SELECT id, title FROM tasks WHERE status = ? AND statusChangedAt < ?`).all(cond.status, cutoff);
        }
        if (tasks.length > 0) {
            // Apply tag if specified
            if (policy.action.tag) {
                for (const t of tasks) {
                    const existing = db.prepare('SELECT tags FROM tasks WHERE id = ?').get(t.id);
