...
export function registerUpdate(program) {
    program
        ...
        if (opts.danger)
            updates.danger = opts.danger;
        if (opts.list)
            updates.list = opts.list;
        if (opts.agent)
            updates.assignedTo = opts.agent === 'none' ? null : opts.agent;
        if (opts.due)
            updates.dueDate = opts.due === 'none' ? null : opts.due;
        if (opts.tags)
            updates.tags = opts.tags.split(',').map((t) => t.trim());
        if (opts.project)
            updates.projectId = opts.project === 'none' ? null : opts.project;
        if (opts.parent)
            updates.parentId = opts.parent === 'none' ? null : opts.parent;
        if (opts.estimate)
            updates.estimatedMinutes = parseInt(opts.estimate);
        if (opts.blocker)
            updates.blockerDescription = opts.blocker;
